...
#             info = l_Info()  # 게시판의 객체 생성 : 유효성 검사가 통과된 데이터를 저장하기 위함
#             info.l_category = form.cleaned_data['category']
#             info.l_title = form.cleaned_data['title']
#             info.l_contents = form.cleaned_data['contents']
#             info.l_writer = Bcuser.objects.get(email=request.session.get('user')) # 로그인한 id 데이터베이스에 저장
#             info.save()

#             return redirect('/info_board/')
#     else:
#         form = l_InfoForm()  # 접속은 했으나 유효성 검사를 안했으므로 다시 유효성 검사부터 진행
#     # 실패시 처음으로 돌아가서 다시해봐
#     return render(request, 'l_info_write.html', {'form': form})